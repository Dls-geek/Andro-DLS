...
def pull_file(config: AppConfig) -> None:
    ...
    out_dir = ensure_config_dir(config, "pull_location", "Downloads")
...
def _pull_directory(
    config: AppConfig, remote_path: str, label: str, category: str | None = None
) -> None:
    """Generic helper to pull a directory from the device.

    category=None → base root; adb names the folder after the remote dir
    (e.g. WhatsApp/). Otherwise the pulled dir lands under root/<category>/
    (e.g. Media/Camera/).
    """
    out_dir = ensure_config_dir(config, "pull_location", category)
...
def copy_screenshots(config: AppConfig) -> None:
    ...
    _pull_directory(config, location, "Screenshots", "Media")
...
def copy_camera(config: AppConfig) -> None:
    ...
    _pull_directory(config, "/sdcard/DCIM/Camera", "Camera Photos", "Media")
